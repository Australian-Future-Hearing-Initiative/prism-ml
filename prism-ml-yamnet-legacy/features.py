...
# ==============================================================================
"""
This module computes YAMNet features.
"""
...
def waveform_to_log_mel_spectrogram_patches(waveform, params):
    ...
    with tf.name_scope("log_mel_features"):
        ...
        # Convert waveform into spectrogram using a Short-Time
        # Fourier Transform.
        # Note that tf.signal.stft() uses a periodic Hann window
        # by default.
        ...
        # magnitude_spectrogram has shape
        # [<# STFT frames>, num_spectrogram_bins]
        ...
        # log_mel_spectrogram has shape
        # [<# STFT frames>, params.mel_bands]

        # Frame spectrogram (shape [<# STFT frames>, params.mel_bands])
        # into patches (the input examples). Only complete frames
        # are emitted, so if there is less than params.patch_window_seconds
        # of waveform then nothing is emitted
        # (to avoid this, zero-pad before processing).
        spectrogram_hop_length_samples = int(
            round(params.sample_rate * params.stft_hop_seconds)
        )
        spectrogram_sample_rate = (
            params.sample_rate / spectrogram_hop_length_samples
        )
        ...
        # features has shape
        # [<# patches>, <# STFT frames in an patch>, params.mel_bands]

        return log_mel_spectrogram, features


def pad_waveform(waveform, params):
    """Pads waveform with silence if needed to get an
    integral number of patches."""
    # In order to produce one patch of log mel spectrogram
    # input to YAMNet, we need at least one patch window
    # length of waveform plus enough extra samples
    ...
    min_num_samples = tf.cast(
        min_waveform_seconds * params.sample_rate, tf.int32
    )
    num_samples = tf.shape(waveform)[0]
    num_padding_samples = tf.maximum(0, min_num_samples - num_samples)

    # In addition, there might be enough waveform for
    # one or more additional patches formed by hopping
    # forward. If there are more samples than one patch,
    ...
    hop_samples = tf.cast(
        params.patch_hop_seconds * params.sample_rate, tf.int32
    )
...
def _tflite_stft_magnitude(signal, frame_length, frame_step, fft_length):
        ...
        return tf.reshape(
            tf.constant(
                (
                    0.5
                    - 0.5
                    * np.cos(2 * np.pi * np.arange(0, 1.0, 1.0 / frame_length))
                ).astype(np.float32),
                name="hann_window",
            ),
            [1, frame_length],
    ...
    def _dft_matrix(dft_length):
        ...
        return np.exp(
            omega * np.outer(np.arange(dft_length), np.arange(dft_length))
        )

    def _rdft(framed_signal, fft_length):
        """Implement real-input Discrete Fourier Transform by matmul."""
        # We are right-multiplying by the DFT matrix, and we are
        # keeping only the first half ("positive frequencies").
        # So discard the second half of rows, but transpose the array
        # for right-multiplication.  The DFT matrix is symmetric,
        # so we could have done it more directly, but this reflects our
        # intention better.
