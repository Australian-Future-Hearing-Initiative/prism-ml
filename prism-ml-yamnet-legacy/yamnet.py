...
# ==============================================================================
"""
This module contains YAMNet definitions.
"""
...
def _conv(name, kernel, stride, filters, params):
    def _conv_layer(layer_input):
        ...
        output = _batch_norm(f"{name}/conv/bn", params)(output)
...
def _separable_conv(name, kernel, stride, filters, params):
    def _separable_conv_layer(layer_input):
        ...
        output = _batch_norm(f"{name}/depthwise_conv/bn", params)(output)
        ...
        output = _batch_norm(f"{name}/pointwise_conv/bn", params)(output)
...
def yamnet(features, params):
    ...
    for i, (layer_fun, kernel, stride, filters) in enumerate(
        _YAMNET_LAYER_DEFS
    ):
        layer_id = i + 1
        net = layer_fun(f"layer{layer_id}", kernel, stride, filters, params)(
            net
        )
    embeddings = layers.GlobalAveragePooling2D()(net)
    logits = layers.Dense(units=params.num_classes, use_bias=True)(embeddings)
    predictions = layers.Activation(activation=params.classifier_activation)(
        logits
    )
...
def yamnet_frames_model(params):
    ...
    Returns:
        ...
        - predictions: (num_patches, num_classes) matrix of class
        scores per time frame
        - embeddings: (num_patches, embedding size) matrix of
        embeddings per time frame
        - log_mel_spectrogram: (num_spectrogram_frames, num_mel_bins)
        spectrogram feature matrix
    """
    waveform = layers.Input(
        batch_shape=(params.input_length,), dtype=tf.float32
    )
    waveform_padded = features_lib.pad_waveform(waveform, params)
    log_mel_spectrogram, features = (
        features_lib.waveform_to_log_mel_spectrogram_patches(
